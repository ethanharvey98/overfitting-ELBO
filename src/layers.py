...
class RandomFourierFeatures(torch.nn.Module):
    def __init__(
        ...
        self.rank = rank
        self.register_buffer("feature_weight", torch.randn(self.rank, in_features, dtype=torch.float64))
        self.register_buffer("feature_bias", 2 * torch.pi * torch.rand(self.rank, dtype=torch.float64))
